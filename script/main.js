...
const animationTimeline = () => {
    ...
    // split chars of chatbox + HBD title
    ...
    // -------------------------------
    // MANUAL ANIMATIONS FOR IDEAS 1–5
    // -------------------------------

    tl.from(".idea-1", 0.7, ideaTextTrans)
    ...
    // -------------------------------
    // TYPING + DISAPPEAR FOR IDEAS 6–17
    // -------------------------------
    for (let i = 6; i <= 17; i++) {
        const elem = document.querySelector(`.idea-${i}`);
        const rawText = elem.textContent.trim();

        // wrap characters in spans
        elem.innerHTML = `<span>${rawText.split("").join("</span><span>")}</span>`;

        // typing animation
        tl.staggerFrom(`.idea-${i} span`, 0.01, { opacity: 0 }, 0.05);

        // fade out the full idea box
        tl.to(`.idea-${i}`, 0.7, ideaTextTransLeave, "+=2");
    }

    // -------------------------------
    // FINAL ANIMATIONS: balloons, hat, pic, wishes
    // -------------------------------

    tl.staggerFromTo(".baloons img", 2.5, { opacity: 0.9, y: 1400 },
